...
function injectParams(htmlContent, params) {
  ...
  // ── NORMALIZE GAME FRAME + BOTTOM BORDER FIX (HOKKA PANEL cut issue) ──
  ...
  // Bottom cut fix - NO visual border, sirf safe-area positioning (green border artifact hataya)
  const bottomFixCss = `<style id="mizanmod-bottom-safe-fix">
#target-game-frame, #gameIframe { height: calc(100% - 16px) !important; bottom: 8px !important; padding-bottom: env(safe-area-inset-bottom, 0px) !important; box-sizing: border-box !important; }
#panel, .panel, [id*="panel" i], [class*="prediction" i], [class*="hokka" i] { margin-bottom: 12px !important; max-height: calc(100vh - 24px) !important; overflow-y: auto !important; -webkit-overflow-scrolling: touch !important; }
.buttons, [class*="buttons" i], [id*="buttons" i] { padding-bottom: env(safe-area-inset-bottom, 8px) !important; margin-bottom: 4px !important; }
body { padding-bottom: env(safe-area-inset-bottom, 0px) !important; }
</style>`;
  if (/<\/head>/i.test(html)) html = html.replace(/<\/head>/i, `${bottomFixCss}</head>`);
  else html = bottomFixCss + html;
