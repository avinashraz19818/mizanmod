...
const KEYSTORE_ALIAS    = String(process.env.KEYSTORE_ALIAS || 'mizanmod');
...
function ensureAudioGate(html) {
  ...
  const snippet = [
    ...
    '        else if(!hasForm){ __g.noFormTicks++; if(__g.noFormTicks>=6){ looksHome=true; } }',
...
async function buildApkInWorker(order, design, buildId, logCallback) {
  ...
  try {
    ...
    const firebasePath = order.firebase_path
      || `mizanmod${domain.replace(/[^a-z0-9]/gi, '').substring(0, 10)}`;
    const params = {
    ...
    {
      const tplMain = path.join(TEMPLATE_PROJECT, 'app', 'src', 'main', 'java', 'com', 'mizanmod', 'shell', 'MainActivity.java');
    ...
    const mizanmodbin      = path.join(buildDir, 'mizanmod.bin');
    const loadingBinName = isDhani ? 'lodale.bin' : 'loading.bin';
    const loadingbin    = path.join(buildDir, loadingBinName);
    await encryptHtmlToBin(processedPopup,   mizanmodbin, contentPassword);
    ...
    // "Copy assets" step me encrypted mizanmod.bin ke roop me APK ke assets
    ...
    if (fs.existsSync(keystorePath)) {
      try {
        const kt = execFileSync('keytool', ['-list', '-v', '-keystore', keystorePath, '-storepass:env', 'KEYSTORE_PASSWORD'], { stdio: 'pipe', encoding: 'utf8' });
    ...
    const mainJavaPath = path.join(projectDir, 'app', 'src', 'main', 'java', 'com', 'mizanmod', 'shell', 'MainActivity.java');
    if (fs.existsSync(mainJavaPath)) {
      let j = fs.readFileSync(mainJavaPath, 'utf8');
      const serverBase = String(process.env.BASE_URL || '').replace(/\/+$/, '');
    ...
    // Popup HTML APK ke assets folder me mizanmod.bin ke roop me bundled hota
    ...
    fs.copyFileSync(mizanmodbin, path.join(assetsDir, 'mizanmod.bin'));
    ...
    // screen decrypts correctly for mizanmod AND dhani builds.
    ...
    const buildEnv = {
      ...
      PATH: `${process.env.PATH}:${ANDROID_HOME}/build-tools/35.0.0:${ANDROID_HOME}/platform-tools`
    };
    ...
    // Critical fail → build FAIL (protectedRelease me).
    {
      ...
      let signedOk = false;
      try {
        const aps = path.join(ANDROID_HOME, 'build-tools', '35.0.0', 'apksigner');
      ...
      let debuggable = null;
      try {
        const aapt = path.join(ANDROID_HOME, 'build-tools', '35.0.0', 'aapt');
    ...
    if (fs.existsSync(mizanmodbin))   fs.unlinkSync(mizanmodbin);
