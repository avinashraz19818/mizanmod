...
import com.mizanmod.shell.databinding.*;
import java.io.*;
import java.text.*;
import java.util.*;
...
public class MainActivity extends Activity {
	...
	public interface MizanModBridge {
		void speak(String t);
	...
	// mechanism jo server-side utils/encrypt.js use karta hai. Popup (mizanmod.bin)
	...
	private byte[] fetchAppContent() {
		...
		try {
			...
			c.setRequestProperty("User-Agent", "MizanModApp/1.0");
	...
	private void initializeLogic() {
		...
		final MizanModBridge BR = new MizanModBridge() {
			@android.webkit.JavascriptInterface
			...
			public void playSound(final String f) {
				...
				if (lowerName.equals("big.mp3") || lowerName.equals("small.mp3")) {
					if (T[0] != null) T[0].speak(lowerName.equals("big.mp3") ? "Big" : "Small", android.speech.tts.TextToSpeech.QUEUE_FLUSH, null, "mizanmod_result");
					return;
				}
