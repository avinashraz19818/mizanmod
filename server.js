...
const SESSION_COOKIE_NAME = COOKIE_SECURE ? '__Host-mizanmod.sid' : 'mizanmod.sid';
...
function normalizeDesignCategory(value, design = {}) {
  ...
  // Old "normal" and "java" categories are both the common MizanMod Java.
  return 'mizanmod';
}
...
app.post('/api/order', requireAuth, iconUpload.single('icon'), async (req, res) => {
  ...
  if (isBoth && cleanFakeRegisterUrl) {
    ...
    fakeFirebasePath = `mizanmodf${fakeDomain.replace(/[^a-z0-9]/gi, '').substring(0, 8)}${orderId}`;
  }
      ...
      if (isBoth && cleanFakeRegisterUrl) {
        ...
        const fakeOrder = makeFakeOrder(order, cleanFakeRegisterUrl,
          order.fake_firebase_path
            || `mizanmodf${extractDomain(cleanFakeRegisterUrl).replace(/[^a-z0-9]/gi,'').substring(0,8)}`,
          1, design);
...
function makeFakeSitePath(registerUrl, siteId) {
  ...
  return `mizanmodf${extractDomain(registerUrl).replace(/[^a-z0-9]/gi, '').substring(0, 8)}s${siteId}`;
}
...
app.get('/api/admin/firebase/selftest', async (req, res) => {
  ...
  try {
    ...
    const u = `${dbUrl}/mizanmod_healthcheck/config.json?access_token=${encodeURIComponent(token)}`;
...
app.get('/api/admin/orders', requireAdmin, (req, res) => {
  ...
  // Get orders
  const rows = db.prepare(`
    SELECT o.*,COALESCE(u.username, 'Unknown') as username,COALESCE(d.name, 'Default') as design_name,COALESCE(d.category, 'mizanmod') as category FROM orders o
    LEFT JOIN users u ON o.user_id=u.id
    LEFT JOIN designs d ON o.design_id=d.id
    ${where}
...
function getOrderFirebaseTarget(order, variant = 'real') {
  ...
  if (v === 'fake') {
    ...
    const firebasePath = order.fake_firebase_path
      || `mizanmodf${domain.replace(/[^a-z0-9]/gi, '').substring(0, 8)}`;
    return {
      variant: 'fake',
      fakeNumber: 1,
      firebasePath,
...
function rebuildOrderInBackground(orderId, rebuildFake = true) {
  ...
  (async () => {
    ...
    if (order.firebase_path) {
      ...
      if (clash) {
        const np = `mizanmod${String(order.domain || '').replace(/[^a-z0-9]/gi, '').substring(0, 8)}${order.id}`;
    ...
    if (order.fake_firebase_path && order.fake_register_url) {
      ...
      if (clash) {
        const np = `mizanmodf${String(extractDomain(order.fake_register_url)).replace(/[^a-z0-9]/gi, '').substring(0, 8)}${order.id}`;
    ...
    if (rebuildFake && order.fake_register_url) {
      ...
      const fakeOrder = makeFakeOrder(order, order.fake_register_url,
        order.fake_firebase_path || `mizanmodf${extractDomain(order.fake_register_url).replace(/[^a-z0-9]/gi,'').substring(0,8)}`,
        1, design);
